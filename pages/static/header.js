import { Avatar, Button, Container, Group, Stack, Text, UnstyledButton, createStyles } from '@mantine/core';
...
export default function Header() {
    ...
    if (status === 'authenticated') {
        return (
            <div>
<Group position='appart'>

                <Container className={classes.user} m={0}>
                    <Group>
                        <Avatar src={data.user.image} alt={data.user.name + ' photo'} radius="xl" />
    ...
    return (
        <Group>
            <Button leftIcon={<GoogleIcon />} variant="default" color="gray" onClick={() => signIn('google', { callbackUrl: "/newuser" })} >sign in with gooogle</Button>
            <Nav></Nav>
        </Group>
    );
}
